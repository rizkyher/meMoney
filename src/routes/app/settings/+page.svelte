...
<section class="mx-auto max-w-3xl space-y-5">
  ...
  <div class="surface-panel space-y-4 p-4 md:p-5">
    <div class="list-row p-4"><p class="field-label">Profile</p><p class="font-black">{data.user.name}</p><p class="text-sm text-muted">{data.user.email}</p></div>
    ...
    <label class="list-row flex items-center gap-3 p-3 text-sm font-bold"><input type="checkbox" /> <ShieldCheck size={18} /> AI scan enabled (default off)</label>
    <label class="list-row flex items-center gap-3 p-3 text-sm font-bold"><input type="checkbox" /> Save receipt default</label>
